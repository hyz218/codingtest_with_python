...
def delivery_chicken():
    ...
    candidates = list(combinations(chicken,m)) #치킨집 m개 조합
    
    def get_sum(candidate): #치킨 거리의 합 계산 함수
        result = 0 #모든 집에 대하여
        for hx,hy in house: #가장 가까운 치킨집 찾기
            temp = 1e9
            for cx, cy in candidate:
                temp = min(temp,abs(hx-cx)+abs(hy-cy)) #가장 가까운 치킨집까지 거리 더하기
            result+=temp 
        return result #치킨 거리의 합 반환

    result = 1e9 #치킨 거리의 합 최소를 찾아 출력
    for candidate in candidates:
        result = min(result,get_sum(candidate))

    return result

#print(delivery_chicken())

from itertools import permutations
def exteriorwall_inspection():

    def solution(n, weak, dist): 
        answer = 0
        length = len(weak) #길이를 2배로 늘려 원형을 일자 형태로 변형
        for i in range(length):
            weak.append(weak[i]+n)
        answer = len(dist)+1 #투입할 친구 수의 최솟값을 찾아야하므로 len(dist)+1로 초기화
        for start in range(length): #0부터 length-1으로 위치를 각각 시작점으로 설정
    ...
    return solution(12, [1,5,6,10], [1,2,3,4])

#print(exteriorwall_inspection())